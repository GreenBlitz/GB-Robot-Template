...
public class Phoenix6SignalBuilder {
    ...
    public static Phoenix6AngleSignal generatePhoenix6Signal(
            StatusSignal<?> signal, double frequency, AngleUnit angleUnit) {
        StatusSignal<?> signalClone = cloneWithFrequency(signal, frequency);
    ...
    public static Phoenix6LatencySignal generatePhoenix6Signal(
            StatusSignal<?> signal, SignalGetter signalSlope, double frequency, AngleUnit angleUnit) {
        StatusSignal<?> signalClone = cloneWithFrequency(signal, frequency);
    ...
    /**
     * Use this function only if you are not fetching the slope signal. Ex: If you care only about position and not
     * velocity do: Phoenix6BothLatencySignal position =
     * Phoenix6SignalBuilder.generatePhoenix6Signal(motor.getPosition(), motor.getVelocity(),...);
     */
    public static Phoenix6LatencyAndSlopeSignal generatePhoenix6Signal(
            StatusSignal<?> signal, StatusSignal<?> signalSlope, double frequency, AngleUnit angleUnit) {
        StatusSignal<?> signalClone = cloneWithFrequency(signal, frequency);
