...
public class LimelightStdDevCalculations {
	...
	public static Supplier<StandardDeviations2D> getMT1StdDevsCalculation(Limelight limelight, StandardDeviations2D minStdDevs, StandardDeviations2D stdDevFactors) {
		return () -> averageTagDistanceParabola(limelight.getMT1RawData().avgTagDist, minStdDevs, stdDevFactors);
	}

	public static Supplier<StandardDeviations2D> getMT2StdDevsCalculation(Limelight limelight, StandardDeviations2D minStdDevs, StandardDeviations2D stdDevFactors) {
	...
	private static StandardDeviations2D averageTagDistanceParabola(
		...
		return new StandardDeviations2D(
			...
			Math.max(minStandardDeviations.angleStandardDeviations(), standardDeviationFactors.angleStandardDeviations() * averageTagDistanceSquared)
		);
	}
