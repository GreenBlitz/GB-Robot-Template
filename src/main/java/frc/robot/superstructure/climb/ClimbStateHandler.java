...
public class ClimbStateHandler {
    ...
    public Command setState(ClimbState state) {
        return switch (state) {
            case STOP ->
                    lifterStateHandler.setState(LifterState.HOLD).alongWith(solenoidStateHandler.setState(SolenoidState.OFF));
            case EXTEND -> (
                    ...
                    .andThen(
                            lifterStateHandler.setState(LifterState.EXTENDED)
                                    .alongWith(solenoidStateHandler.setState(SolenoidState.HOLD).raceWith(new WaitCommand(7)).andThen(solenoidStateHandler.setState(SolenoidState.OFF)))
                    );
            case RETRACT ->
                    lifterStateHandler.setState(LifterState.RETRACTED).alongWith(solenoidStateHandler.setState(SolenoidState.OFF));
        };
    }
