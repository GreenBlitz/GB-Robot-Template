...
public class OdometryVisualization {
    ...
    public OdometryVisualization(String logPath, Supplier<Transform2d> deltaA, Supplier<Transform2d> deltaB, Supplier<Transform2d> deltaC, Supplier<Transform2d> deltaD, GridPlacements gridPlacements) {
    ...
    private Translation2d getAbsoluteCenter() {
        return gridPlacements.TOP_LEFT().plus(gridPlacements.TOP_RIGHT()).plus(gridPlacements.BOTTOM_RIGHT()).plus(gridPlacements.BOTTOM_LEFT()).div(4d);
    }

    private GridPlacements getPlacementsAfterChange() {
        return this.gridPlacements.plus(
                new GridPlacements(deltaA.get().getTranslation(), deltaB.get().getTranslation(), deltaC.get().getTranslation(), deltaD.get().getTranslation())
