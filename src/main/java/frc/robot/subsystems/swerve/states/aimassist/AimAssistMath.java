...
public class AimAssistMath {
    ...
    public static ChassisSpeeds getRotationAssistedSpeeds(
        ...
        Rotation2d pidOutputVelocityPerSecond = Rotation2d
            .fromDegrees(swerveConstants.rotationDegreesPIDController().calculate(
                robotHeading.getDegrees(),
                SwerveMath.getSqrtPIDSetPoint(robotHeading, targetHeading).getDegrees()
            ));

        Rotation2d angularVelocityPerSecond = applyMagnitudeCompensation(
    ...
    private static Rotation2d applyMagnitudeCompensation(Rotation2d velocityPerSecond, double magnitude) {
        return velocityPerSecond.times(SwerveConstants.AIM_ASSIST_MAGNITUDE_FACTOR)
                                .div(magnitude + SwerveConstants.AIM_ASSIST_MAGNITUDE_FACTOR);
    }
