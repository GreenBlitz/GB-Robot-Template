...
public class KrakenX60ArmBuilder {
	...
	protected static Arm build(String logPath) {
		...
		TalonFXMotor motor = new TalonFXMotor(logPath, IDs.TalonFXIDs.ARM_MOTOR, buildSysidConfig(), buildArmSimulation());
	...
	private static TalonFXConfiguration buildTalonFXConfiguration() {
		...
		config.Feedback.FeedbackRemoteSensorID = IDs.CANCodersIDs.ARM_ENCODER.id();
	...
	private static IAngleEncoder getEncoder(String logPath) {
		return switch (Robot.ROBOT_TYPE) {
			case REAL ->
				new CANCoderEncoder(
					logPath + "/Encoder",
					new CANcoder(IDs.CANCodersIDs.ARM_ENCODER.id(), IDs.CANCodersIDs.ARM_ENCODER.busChain().getChainName())
				);
