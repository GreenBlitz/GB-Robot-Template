...
public class AngleMath {
	...
	public static double getAngleAverageWrapped(double sinSum, double cosSum, int posesAmount) {
		return Math.atan2(sinSum / posesAmount, cosSum / posesAmount);
	}
	...
	public static Rotation3d getAngleAverageWrapped(
		...
		return new Rotation3d(
			getAngleAverageWrapped(sinXSum, cosXSum, posesAmount),
			getAngleAverageWrapped(sinYSum, cosYSum, posesAmount),
			getAngleAverageWrapped(sinZSum, cosZSum, posesAmount)
		);
	}
	...
	public static Pose3d getCameraAngle(Pose3d tagToRobot, Pose3d tagToCamera) {
		Pose3d rotatedPose = new Pose3d().rotateAround(tagToCamera.getTranslation(), tagToRobot.getRotation().minus(tagToCamera.getRotation()));
		return new Pose3d(
			rotatedPose.getTranslation().plus(tagToRobot.getTranslation().minus(tagToCamera.getTranslation())),
			rotatedPose.getRotation()
		);
	}
