...
public class Swerve extends GBSubsystem {
	...
	public Translation3d getAccelerationFromIMUMetersPerSecondSquared() {
		return imuSignals.getLatestAccelerationG().times(RobotConstants.G_METERS_PER_SECOND_SQUARED_ISRAEL);
	}
	...
	public OdometryData[] getAllOdometryData() {
		...
		for (int i = 0; i < odometryData.length; i++) {
			odometryData[i] = new OdometryData(
				...
				imu instanceof EmptyIMU ? Optional.empty() : Optional.of(getIMUAccelerationMetesPerSecondSquared())
			);
		}
	...
	public double getIMUAccelerationMetesPerSecondSquared() {
