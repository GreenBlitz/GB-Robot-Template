...
public class LEDStateHandler extends GBSubsystem {
	...
	public LEDStateHandler(String logPath, CANdle candle) {
		...
		setDefaultCommand(new ConditionalCommand(setState(LEDState.IDLE), setState(LEDState.DISABLE), DriverStation::isEnabled).ignoringDisable(true));
	}

	public Command setState(LEDState state) {
		Command setStateCommand = asSubsystemCommand(
			new InitExecuteCommand(() -> candle.animate(state.getAnimation()), () -> {}, this).withTimeout(LEDConstants.CORAL_IN_BLINK_TIME_SECONDS).ignoringDisable(true),
			state.name()
		);
