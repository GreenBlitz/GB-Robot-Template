...
public class Pigeon2Wrapper extends Pigeon2 {
    ...
    /**
     * This function only set pitch for the function "getAdjustedRoll". If you want to use the pitch signal you need to
     * use "getRoll" but note to yourself that it won't be affected by "setRoll". If for some reason you want to get the
     * needed offset to make the returned value affected by resets use "getRollOffset".
     *
     * @param newRoll - the wanted roll of the gyro to have
     */
    public void setRoll(Rotation2d newRoll) {
        rollOffSetDegrees =
                calculateOffset(newRoll.getDegrees(), getRoll().getValue().in(Degrees));
    }

    /**
     * This function only set pitch for the function "getAdjustedPitch". If you want to use the pitch signal you need to
     * use "getPitch" but note to yourself that it won't be affected by "setPitch". If for some reason you want to get
     * the needed offset to make the returned value affected by resets use "getPitchOffset".
     *
     * @param newPitch - the wanted pitch of the gyro to have
     */
    public void setPitch(Rotation2d newPitch) {
        pitchOffSetDegrees =
                calculateOffset(newPitch.getDegrees(), getPitch().getValue().in(Degrees));
    }
