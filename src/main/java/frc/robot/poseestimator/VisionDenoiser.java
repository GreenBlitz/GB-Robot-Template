...
public class VisionDenoiser {
	...
	private LinkedList<VisionObservation> fixAccordingToOdometry(Pose2d currentPose, Pose2d odometryPose, TimeInterpolatableBuffer<Pose2d> odometryPoseInterpolator) {
		...
		for (VisionObservation observation : observations) {
			...
			odometryFix.ifPresent(odometryPoseSample -> {
				Pose2d currentEstimation = PoseEstimationMath
					.combineVisionToOdometry(observation, odometryPoseSample, currentPose, odometryPose, PoseEstimatorConstants.DEFAULT_ODOMETRY_STANDARD_DEVIATIONS);
				Pose2d fixedPose = new Pose2d(currentEstimation.getTranslation(), odometryPoseSample.getRotation());
				output.add(new VisionObservation(
					fixedPose,
	...
	public Optional<VisionObservation> calculateAverageFixedPose(Pose2d currentPose, Pose2d odometryPose, TimeInterpolatableBuffer<Pose2d> odometryPoseInterpolator) {
		return calculateAverage(fixAccordingToOdometry(currentPose, odometryPose, odometryPoseInterpolator));
	}

	public Optional<VisionObservation> calculateWeightedAverageFixedPose(Pose2d currentPose, Pose2d odometryPose, TimeInterpolatableBuffer<Pose2d> odometryPoseInterpolator) {
