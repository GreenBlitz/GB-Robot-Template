...
//		usedJoystick.POV_LEFT.onTrue(robot.getRobotCommander().setState(RobotState.PRE_CLIMB_WITH_AIM_ASSIST));
	...
	private static void secondJoystickButtons(Robot robot) {
		...
		usedJoystick.START.onTrue(
			new InstantCommand(
				() -> robot.getRobotCommander()
					.getSuperstructure().driverIsAlgaeInAlgaeIntakeOverride = !robot.getRobotCommander()
						.getSuperstructure().driverIsAlgaeInAlgaeIntakeOverride
			)
		);
